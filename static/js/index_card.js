...
class CardView {
  ...
  async render(data) {
    ...
    if (current) {
      ...
      // 提取溫度範圍
      const temperatureRange = this.extractTemperature(current.values.WeatherDescription);
      ...
      // 設置天氣圖標
      this.element.currentWeatherIcon.src = this.getWeatherIcon(current.values.WeatherDescription);
    ...
    for (let i = 0; i < forecastData.length; i++) {
      ...
      if (forecast && forecastElements) {
        ...
        if (forecastElements.icon) {
          forecastElements.icon.src = this.getWeatherIcon(forecast.values.WeatherDescription);
        }
        
        // 設置溫度
        if (forecastElements.temp) {
          forecastElements.temp.innerHTML = this.extractTemperature(forecast.values.WeatherDescription);
        }
        
        // 格式化並設置日期
        if (forecastElements.date) {
          const date = new Date(forecast.date);
          forecastElements.date.textContent = `${date.getDate()} ${date.toLocaleString('en-US', { month: 'short' })}`;
        }
        
        // 設置星期幾
        if (forecastElements.weekday) {
          const date = new Date(forecast.date);
          forecastElements.weekday.textContent = date.toLocaleString('zh-TW', { weekday: 'long' });
        }
      }
    }
  ...
  extractTemperature(description) {
    const tempRange = description.match(/溫度攝氏(\d+)[\u4e00-\u9fa5]*?(\d+)度/);
    if (tempRange) {
